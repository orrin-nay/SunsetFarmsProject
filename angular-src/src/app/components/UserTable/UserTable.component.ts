...
export class UserTableComponent {
  ...
  changeEmail(emailModal: ModalComponent): void {
    ...
    if (this.validateService.validateEmail(this.newEmail)) {
      this.authService.changeEmailAdmin(this.newEmail, this.user.id).subscribe(data => {
        if (data.data.ChangeEmailAdmin.success) {
          this.user.email = this.newEmail;

          this.flashMessage.show(data.data.ChangeEmailAdmin.success, { cssClass: 'alert-success', timeout: 12000 });
          emailModal.hide();
        } else {
  ...
  changePhone(phoneModal: ModalComponent): void {
    ...
    if (this.validateService.validatePhone(this.newPhone)) {
      ...
      this.authService.changePhoneAdmin(this.newPhone, this.user.id).subscribe(data => {
        if (data.data.ChangePhoneAdmin.success) {
          this.user.phone = this.newPhone;

          this.flashMessage.show(data.data.ChangePhoneAdmin.success, { cssClass: 'alert-success', timeout: 12000 });
          phoneModal.hide();
        } else {
  ...
  changeDOB(dOBModal: ModalComponent): void {
    ...
    if (this.validateService.validateDate(this.newDOB)) {
      this.authService.changeDOBAdmin(this.newDOB, this.user.id).subscribe(data => {
        if (data.data.ChangeDOBAdmin.success) {
          this.user.dOB = this.newDOB;

          this.flashMessage.show(data.data.ChangeDOBAdmin.success, { cssClass: 'alert-success', timeout: 12000 });
          dOBModal.hide();
        } else {
  ...
  changeGender(genderModal: ModalComponent): void {
    ...
    if (this.validateService.validateGender(this.newGender)) {
      this.authService.changeGenderAdmin(this.newGender, this.user.id).subscribe(data => {
        if (data.data.ChangeGenderAdmin.success) {
          this.user.gender = this.newGender;

          this.flashMessage.show(data.data.ChangeGenderAdmin.success, { cssClass: 'alert-success', timeout: 12000 });
          genderModal.hide();
        } else {
  ...
  changeHOH(hOHModal: ModalComponent): void {
    ...
    if (!this.validateService.validateHOH(this.newHOH === 'true')) {
      this.authService.changeHOHAdmin(this.newHOH, this.user.id).subscribe(data => {
        if (data.data.ChangeHOHAdmin.success) {
          this.user.hOH = this.newHOH === 'true';

          this.flashMessage.show(data.data.ChangeHOHAdmin.success, { cssClass: 'alert-success', timeout: 12000 });
          hOHModal.hide();
        } else {
  ...
  changeAdminPrivilege(AdminModal: ModalComponent): void {
    if (this.newHOH === undefined) {
      ...
      this.authService.changeAdminPrivilege(false, this.user.id).subscribe(data => {
        if (data.data.ChangeAdminPrivilege.success) {
          this.user.admin = this.newAdmin === 'true';

          this.flashMessage.show(data.data.ChangeAdminPrivilege.success, { cssClass: 'alert-success', timeout: 12000 });
          AdminModal.hide();
        } else {
  ...
  async removeSkill(skill: string): Promise<void> {
    ...
    if (index !== -1) {
      this.authService.removeSkillAdmin(skill, this.user.id).subscribe(async (data) => {
        console.log(data);
  ...
  addSkill(skillsModal: ModalComponent): void {
    if (this.availableSkills.indexOf(this.newSkill) !== -1) {
      this.authService.addSkillAdmin(this.newSkill, this.user.id).subscribe(async (data) => {
        console.log(data);
