...
export class ManageUsersComponent implements OnInit {
  ...
  private router: Router) {
    this.skillsService.getSkillsList().then(skills=>{
      this.posibleFilterSuggestions = skills.concat(['Male', 'Female', 'Head Of Household', 'Not Head Of Household']);
    })
    try {
  ...
  updateFilterSujestions(): void {
    ...
    if (this.filters.indexOf('Female') !== -1) {
      this.filterSuggestions.splice(
        this.filterSuggestions.indexOf('Male'),
        1
      );
    }
    ...
    this.authService.searchUsers(this.filterField).subscribe(data => {
      if (data.data.SearchUser.error) {
        this.flashMessage.show(data.data.SearchUser.error, {cssClass: 'alert-danger', timeout: 3000});
      } else if (data.data.SearchUser.results) {
        this.filterSuggestions = this.filterSuggestions.concat(data.data.SearchUser.results);
      }
    });
  }
  }
  getFilterResults(): void {
    this.authService.getFilterResultsNumber([].concat(this.filters)).subscribe(data => {
      if (data.data.FilterResultsNumber.success) {
        ...
        this.flashMessage.show(data.data.ErrorSuccess.error, {cssClass: 'alert-danger', timeout: 3000});
      }
    });
    this.authService.getFilterResults([].concat(this.filters)).subscribe(data => {
  ...
  sendMessage(areYouSureModal: ModalComponent): void {
    ...
    this.authService.sendMessage([].concat(this.filters), localStorage.getItem('subject'),
    localStorage.getItem('message')).then(data => {

      if (data.data.SendMessage.success) {
        this.flashMessage.show(data.data.SendMessage.success, {cssClass: 'alert-success', timeout: 3000});
        localStorage.setItem('subject', '');
        localStorage.setItem('message', '');
        localStorage.setItem('filters', '');
        this.router.navigate(['dashboard/']);
      } else if (data.data.SendMessage.error) {
        this.flashMessage.show(data.data.SendMessage.error, {cssClass: 'alert-danger', timeout: 3000});
      }
    });
  }
}
