...
constructor(private apollo: Apollo,
    ...
    getSkillsList(): Promise<Array<string>> {
        interface SkillsResponse {
          ...
          return new Promise((resolve) => {
            const skills = this.apollo.query<SkillsResponse>({
                query: QuerySkills
              }).subscribe(data => {
                if (data.data.GetSkills.skills) {
                    resolve(data.data.GetSkills.skills);
                } else { 
                    this.flashMessage.show(data.data.GetSkills.error, { cssClass: 'alert-danger', timeout: 12000 });
                }
              });
    ...
    async getSkills(userSkills: Array<String>): Promise<Array<string>> {
        const skills = await this.getSkillsList();
